...
export class DepartmentService {
  ...
  private departmentUrl = this.globals.getBackendUrl() +  'department/';
    ...
    return this.http.get(endPoint)
      .map(res => {
          ...
          for ( i = 0 ; i < response.length ; i++) {
            ...
            department.setName(response[i].name);
            department.setCompanyName(response[i].companyName);
            department.setEmployeeNames(response[i].employeeNames);
